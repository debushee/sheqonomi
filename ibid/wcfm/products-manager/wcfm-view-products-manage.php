...
if(isset($_POST['_auction_item_condition'])){
    update_post_meta( $product_id, '_auction_item_condition', $_POST['_auction_item_condition'] );
}
if(isset($_POST['_auction_type'])){
    update_post_meta( $product_id, '_auction_type', $_POST['_auction_type'] );
}
		...
		if( WCFM_Dependencies::wcfmvm_plugin_active_check() ) {
			...
			<div class="collapse wcfm-collapse">
				<div class="wcfm-page-headig">
					...
					<span class="wcfm-page-heading-text"><?php _e( 'Add Product', 'ibid' ); ?></span>
					<?php do_action( 'wcfm_page_heading' ); ?>
				</div>
				<div class="wcfm-collapse-content wcfm-nolimit-content">
					<div class="wcfm-container">
						<div class="wcfm-clearfix"></div><br />
						<h2><?php _e( 'You have reached your product limit!', 'ibid' ); ?></h2>
						<div class="wcfm-clearfix"></div><br />
	...
	if($product && !empty($product)) {
		...
		if(!empty($gallery_img_ids)) {
			foreach($gallery_img_ids as $gallery_img_id) {
				$gallery_img_urls[]['gimage'] = wp_get_attachment_url($gallery_img_id);
			}
		}
...
$variation_shipping_option_array = array('-1' => __('Same as parent', 'ibid'));
$shipping_option_array = array('_no_shipping_class' => __('No shipping class', 'ibid'));
...
$variation_tax_classes_options['parent'] = __( 'Same as parent', 'ibid' );
$variation_tax_classes_options[''] = __( 'Standard', 'ibid' );
$tax_classes_options[''] = __( 'Standard', 'ibid' );
...
<div class="collapse wcfm-collapse" id="wcfm_products_manage">
  <div class="wcfm-page-headig">
		<span class="wcfmfa fa-cube"></span>
		<span class="wcfm-page-heading-text"><?php _e( 'Manage Product', 'ibid' ); ?></span>
		<?php do_action( 'wcfm_page_heading' ); ?>
	</div>
	<div class="wcfm-collapse-content">
		...
		<div class="wcfm-container wcfm-top-element-container">
			...
			<h2><?php if( $product_id ) { _e('Edit Product', 'ibid' ); } else { _e('Add Product', 'ibid' ); } ?></h2>
			<?php do_action( 'after_wcfm_products_manage_title' ); ?>
			
			<?php
			if( $product_id ) {
				?>
				<span class="product-status product-status-<?php echo esc_attr($wcfm_products_single->post_status); ?>"><?php if( $wcfm_products_single->post_status == 'publish' ) { _e( 'Published', 'ibid' ); } else { echo ucfirst( $wcfm_products_single->post_status ); } ?></span>
				<?php
				if( apply_filters( 'wcfm_is_allow_product_preview', true ) ) {
					if( $wcfm_products_single->post_status == 'publish' ) {
						...
						?>
						<span class="view_count"><span class="wcfmfa fa-eye text_tip" data-tip="<?php _e( 'Views', 'ibid' ); ?>"></span>
						<?php
						echo get_post_meta( $wcfm_products_single->ID, '_wcfm_product_views', true ) . '</span></a>';
					} else {
						echo '<a target="_blank" href="' . apply_filters( 'wcfm_product_preview_url', get_permalink( $wcfm_products_single->ID ) ) . '">';
						?>
						<span class="view_count"><span class="wcfmfa fa-eye text_tip" data-tip="<?php _e( 'Preview', 'ibid' ); ?>"></span>
						<?php
						echo '</a>';
					}
				}
			...
			if( $allow_wp_admin_view = apply_filters( 'wcfm_allow_wp_admin_view', true ) ) {
				if( $product_id ) {
					?>
					<a target="_blank" class="wcfm_wp_admin_view text_tip" href="<?php echo admin_url('post.php?post='.$product_id.'&action=edit'); ?>" data-tip="<?php _e( 'WP Admin View', 'ibid' ); ?>"><span class="fab fa-wordpress fa-wordpress-simple"></span></a>
					<?php
				} else {
					?>
					<a target="_blank" class="wcfm_wp_admin_view text_tip" href="<?php echo admin_url('post-new.php?post_type=product'); ?>" data-tip="<?php _e( 'WP Admin View', 'ibid' ); ?>"><span class="fab fa-wordpress fa-wordpress-simple"></span></a>
					<?php
				}
			}
			
			if( $has_new = apply_filters( 'wcfm_add_new_product_sub_menu', true ) ) {
				echo '<a id="add_new_product_dashboard" class="add_new_wcfm_ele_dashboard text_tip" href="'.get_wcfm_edit_product_url().'" data-tip="' . __('Add New Product', 'ibid') . '"><span class="wcfmfa fa-cube"></span><span class="text">' . __( 'Add New', 'ibid') . '</span></a>';
			}
			...
			if( $product_id && !$wcfm_is_translated_product && apply_filters( 'wcfm_is_allow_duplicate_product', true ) && apply_filters( 'wcfm_is_allow_product_limit', true ) ) {
				if( WCFM_Dependencies::wcfmu_plugin_active_check() ) {
					echo '<a id="wcfm_product_duplicate" class="wcfm_product_duplicate add_new_wcfm_ele_dashboard text_tip" href="#" data-proid="'. $product_id .'" data-tip="' . __('Duplicate Product', 'ibid') . '"><span class="wcfmfa fa-copy"></span><span class="text">' . __( 'Duplicate', 'ibid') . '</span></a>';
				}
			}
				...
				if( apply_filters( 'wcfm_is_allow_featured_product', true ) ) {
					if( WCFM_Dependencies::wcfmu_plugin_active_check() ) {
						if( has_term( 'featured', 'product_visibility', $wcfm_products_single->ID ) ) {
							echo '<a id="wcfm_product_featured" class="wcfm_product_featured add_new_wcfm_ele_dashboard text_tip" href="#" data-featured="nofeatured" data-proid="'. $product_id .'" data-tip="' . __('No Featured', 'ibid') . '"><span class="wcfmfa fa-star-of-life"></span><span class="text">' . __( 'No Featured', 'ibid') . '</span></a>';
						} else {
							if( apply_filters( 'wcfm_has_featured_product_limit', true ) ) {
								echo '<a id="wcfm_product_featured" class="wcfm_product_featured add_new_wcfm_ele_dashboard text_tip" href="#" data-featured="featured" data-proid="'. $product_id .'" data-tip="' . __('Mark Featured', 'ibid') . '"><span class="wcfmfa fa-star"></span><span class="text">' . __( 'Mark Featured', 'ibid') . '</span></a>';
							}
						}
					}
				}
...
"is_virtual" => array('desc' => __('Virtual', 'ibid') , 'type' => 'checkbox', 'class' => 'wcfm-checkbox wcfm_ele wcfm_half_ele_checkbox simple booking non-variable-subscription non-job_package non-resume_package non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element, 'desc_class' => 'wcfm_title wcfm_ele virtual_ele_title checkbox_title simple booking non-variable-subscription non-job_package non-resume_package non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element, 'value' => 'enable', 'dfvalue' => $is_virtual),
"is_downloadable" => array('desc' => __('Downloadable', 'ibid') , 'type' => 'checkbox', 'class' => 'wcfm-checkbox wcfm_ele wcfm_half_ele_checkbox simple appointment non-variable-subscription non-job_package non-resume_package non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element, 'desc_class' => 'wcfm_title wcfm_ele downloadable_ele_title checkbox_title simple appointment non-variable-subscription non-job_package non-resume_package non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element, 'value' => 'enable', 'dfvalue' => $is_downloadable),
"pro_title" => array( 'placeholder' => __('Product Title', 'ibid') , 'type' => 'text', 'class' => 'wcfm-text wcfm_ele wcfm_product_title wcfm_full_ele simple variable external grouped booking', 'value' => $title),
//"visibility"     => array('label' => __('Visibility', 'ibid'), 'type' => 'select', 'options' => array('visible' => __('Catalog/Search', 'ibid'), 'catalog' => __('Catalog', 'ibid'), 'search' => __('Search', 'ibid'), 'hidden' => __('Hidden', 'ibid')), 'class' => 'wcfm-select wcfm_ele wcfm_half_ele wcfm_half_ele_right simple variable external', 'label_class' => 'wcfm_ele wcfm_half_ele_title wcfm_title simple variable external', 'value' => $visibility, 'hints' => __('Choose where this product should be displayed in your catalog. The product will always be accessible directly.', 'ibid'))
						...
						<?php if( !apply_filters( 'wcfm_is_category_checklist', true ) ) { ?>
						  ...
						  <?php if( apply_filters( 'wcfm_is_allow_category', true ) && apply_filters( 'wcfm_is_allow_pm_category', true ) ) { ?>
						  	<?php if( apply_filters( 'wcfm_is_allow_product_category', true ) ) { $ptax_custom_arrtibutes = apply_filters( 'wcfm_taxonomy_custom_attributes', array(), 'product_cat' ); ?>
									<p class="wcfm_title"><strong><?php echo apply_filters( 'wcfm_taxonomy_custom_label', __( 'Categories', 'ibid' ), 'product_cat' ); ?></strong></p><label class="screen-reader-text" for="product_cats"><?php echo apply_filters( 'wcfm_taxonomy_custom_label', __( 'Categories', 'ibid' ), 'product_cat' ); ?></label>
									<select id="product_cats" name="product_cats[]" class="wcfm-select wcfm_ele simple variable external grouped booking" multiple="multiple" data-maximum-selection-length="<?php echo esc_attr($catlimit); ?>" <?php echo implode( ' ', $ptax_custom_arrtibutes ); ?> style="width: 100%; margin-bottom: 10px;">
										<?php
...
if( $product_taxonomy->public && $product_taxonomy->show_ui && $product_taxonomy->meta_box_cb && $product_taxonomy->hierarchical ) {
	if( apply_filters( 'wcfm_is_allow_custom_taxonomy_'.$product_taxonomy->name, true ) ) {
		...
		?>
		<p class="wcfm_title taxonomy_<?php echo esc_attr($product_taxonomy->name); ?>"><strong><?php echo apply_filters( 'wcfm_taxonomy_custom_label', $product_taxonomy->label, $product_taxonomy->name ); ?></strong></p><label class="screen-reader-text" for="<?php echo esc_attr($product_taxonomy->name); ?>"><?php echo apply_filters( 'wcfm_taxonomy_custom_label', $product_taxonomy->label, $product_taxonomy->name ); ?></label>
		<select id="<?php echo esc_attr($product_taxonomy->name); ?>" name="product_custom_taxonomies[<?php echo esc_attr($product_taxonomy->name); ?>][]" class="wcfm-select product_taxonomies wcfm_ele simple variable external grouped booking" multiple="multiple" data-maximum-selection-length="<?php echo esc_attr($taxonomy_limit); ?>" <?php echo implode( ' ', $ptax_custom_arrtibutes ); ?> style="width: 100%; margin-bottom: 10px;">
			<?php
		...
		<?php
							...
							if( $wcfm_is_allow_tags = apply_filters( 'wcfm_is_allow_tags', true ) ) {
								if( apply_filters( 'wcfm_is_tags_input', true ) ) {
									$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_product_simple_fields_tag', array(  "product_tags" => array('label' => __('Tags', 'ibid') , 'type' => 'text', 'class' => 'wcfm-text wcfm_ele simple variable external grouped booking', 'label_class' => 'wcfm_title', 'value' => $product_tags, 'placeholder' => __('Separate Product Tags with commas', 'ibid'), 'hints' => __( 'Product tags are descriptive labels you can add to your products. Popular search engines can use tags to get information about your store. You can add more than one tag separating them with a comma.', 'ibid' ), 'desc' => __( 'Choose from the most used tags', 'ibid' ), 'desc_class' => 'wcfm_full_ele wcfm_fetch_tag_cloud' )
...
if( $product_taxonomy->public && $product_taxonomy->show_ui && $product_taxonomy->meta_box_cb && !$product_taxonomy->hierarchical ) {
	if( apply_filters( 'wcfm_is_allow_custom_taxonomy_'.$product_taxonomy->name, true ) ) {
		...
		$WCFM->wcfm_fields->wcfm_generate_form_field( array(  $product_taxonomy->name => array( 'label' => $product_taxonomy->label, 'name' => 'product_custom_taxonomies_flat[' . $product_taxonomy->name . '][]', 'type' => 'textarea', 'class' => 'wcfm-textarea wcfm_ele wcfm_full_ele simple variable external grouped booking', 'label_class' => 'wcfm_title wcfm_full_ele', 'value' => $taxonomy_values, 'placeholder' => __('Separate Product with commas ' . $product_taxonomy->label, 'ibid') ) ) );
	}
}
											}
										}
			...
			"excerpt" => array('label' => __('Short Description', 'ibid') , 'type' => $wpeditor, 'class' => 'wcfm-textarea wcfm_ele wcfm_full_ele simple variable external grouped booking ' . $rich_editor , 'label_class' => 'wcfm_title wcfm_full_ele ' . $rich_editor, 'rows' => 5, 'value' => $excerpt, 'teeny' => true ),
			"description" => array('label' => __('Description', 'ibid') , 'type' => $wpeditor, 'class' => 'wcfm-textarea wcfm_ele wcfm_full_ele simple variable external grouped booking ' . $rich_editor, 'label_class' => 'wcfm_title wcfm_full_ele ' . $rich_editor, 'rows' => 10, 'value' => $description),
	...
	"gimage" => array( 'type' => 'upload', 'class' => 'wcfm_gallery_upload', 'prwidth' => 75 ),
							...
							if( !WCFM_Dependencies::wcfmu_plugin_active_check() ) {
								if( apply_filters( 'is_wcfmu_inactive_notice_show', true ) ) {
									//wcfmu_feature_help_text_show( __( 'Unlimited Image Gallery', 'ibid' ), false, true );
								}
							}
						}
						?>
							...
							if( apply_filters( 'wcfm_is_allow_category', true ) && apply_filters( 'wcfm_is_allow_pm_category', true ) ) {
								if( apply_filters( 'wcfm_is_allow_product_category', true ) ) {
									...
									<div class="wcfm_product_manager_cats_checklist_fields">
										<p class="wcfm_title wcfm_full_ele"><strong><?php echo apply_filters( 'wcfm_taxonomy_custom_label', __( 'Categories', 'ibid' ), 'product_cat' ); ?></strong></p><label class="screen-reader-text" for="product_cats"><?php echo apply_filters( 'wcfm_taxonomy_custom_label', __( 'Categories', 'ibid' ), 'product_cat' ); ?></label>
										<ul id="product_cats_checklist" class="product_taxonomy_checklist product_taxonomy_checklist_product_cat wcfm_ele simple variable external grouped booking" data-catlimit="<?php echo esc_attr($catlimit); ?>">
											<?php
if ( $product_categories ) {
...
<p class="description wcfm_full_ele wcfm_side_add_new_category wcfm_add_new_category wcfm_add_new_taxonomy">+<?php _e( 'Add new category', 'ibid' ); ?></p>
<div class="wcfm_add_new_taxonomy_form wcfm_add_new_taxonomy_form_hide">
	...
	$args = apply_filters( 'wcfm_wp_dropdown_categories_args', array(
									'show_option_all'    => '',
									'show_option_none'   => __( '-- Parent category --', 'ibid' ),
									'option_none_value'  => '0',
									'hide_empty'         => 0,
									'hierarchical'       => 1,
	...
	?>
	<button type="button" data-taxonomy="product_cat" class="button wcfm_add_category_bt wcfm_add_taxonomy_bt"><?php _e( 'Add', 'ibid' ); ?></button>
	<div class="wcfm_clearfix"></div>
</div>
											</div>
											<div class="wcfm_clearfix"></div>
...
if( $product_taxonomy->public && $product_taxonomy->show_ui && $product_taxonomy->meta_box_cb && $product_taxonomy->hierarchical ) {
	if( apply_filters( 'wcfm_is_allow_custom_taxonomy_'.$product_taxonomy->name, true ) ) {
		...
		<div class="wcfm_product_manager_cats_checklist_fields wcfm_product_taxonomy_<?php echo esc_attr($product_taxonomy->name); ?>">
			<p class="wcfm_title wcfm_full_ele"><strong><?php echo apply_filters( 'wcfm_taxonomy_custom_label', $product_taxonomy->label, $product_taxonomy->name ); ?></strong></p><label class="screen-reader-text" for="<?php echo esc_attr($product_taxonomy->name); ?>"><?php echo apply_filters( 'wcfm_taxonomy_custom_label', $product_taxonomy->label, $product_taxonomy->name ); ?></label>
			<ul id="<?php echo esc_attr($product_taxonomy->name); ?>" class="product_taxonomy_checklist product_custom_taxonomy_checklist product_taxonomy_checklist_<?php echo esc_attr($product_taxonomy->name); ?> wcfm_ele simple variable external grouped booking" data-catlimit="<?php echo esc_attr($taxonomy_limit); ?>">
				<?php
					$product_taxonomy_terms   = get_terms( $product_taxonomy->name, 'orderby=name&hide_empty=0&parent=0' );
					if ( $product_taxonomy_terms ) {
		...
		if( WCFM_Dependencies::wcfmu_plugin_active_check() ) {
			if( apply_filters( 'wcfm_is_allow_add_category', true ) && apply_filters( 'wcfm_is_allow_add_taxonomy', true ) && apply_filters( 'wcfm_is_allow_add_'.$product_taxonomy->name, true ) ) {
				?>
				<div class="wcfm_add_new_category_box wcfm_add_new_taxonomy_box">
					<p class="description wcfm_full_ele wcfm_side_add_new_category wcfm_add_new_category wcfm_add_new_taxonomy">+<?php echo __( 'Add new', 'ibid' ) . ' ' . apply_filters( 'wcfm_taxonomy_custom_label', $product_taxonomy->label, $product_taxonomy->name ); ?></p>
					<div class="wcfm_add_new_taxonomy_form wcfm_add_new_taxonomy_form_hide">
						<?php 
						...
						$args = array(
	'show_option_all'    => '',
	'show_option_none'   => __( '-- Parent taxonomy --', 'ibid' ),
	'option_none_value'  => '0',
	'hide_empty'         => 0,
	'hierarchical'       => 1,
	...
	'taxonomy'           => $product_taxonomy->name,
						...
						wp_dropdown_categories( $args );
						?>
						<button type="button" data-taxonomy="<?php echo esc_attr($product_taxonomy->name); ?>" class="button wcfm_add_category_bt wcfm_add_taxonomy_bt"><?php _e( 'Add', 'ibid' ); ?></button>
						<div class="wcfm_clearfix"></div>
					</div>
				</div>
				<div class="wcfm_clearfix"></div>
				<?php
							...
							if( apply_filters( 'wcfm_is_allow_tags', true ) ) {
								...
								if( apply_filters( 'wcfm_is_tags_input', true ) ) {
									$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_product_simple_fields_tag', array(  "product_tags" => array('label' => __('Tags', 'ibid') , 'type' => 'textarea', 'class' => 'wcfm-textarea wcfm_ele wcfm_full_ele product_tags_ele simple variable external grouped booking', 'label_class' => 'wcfm_title wcfm_full_ele product_tags_ele', 'value' => $product_tags, 'placeholder' => __('Separate Product Tags with commas', 'ibid'), 'hints' => __( 'Product tags are descriptive labels you can add to your products. Popular search engines can use tags to get information about your store. You can add more than one tag separating them with a comma.', 'ibid' ), 'desc' => __( 'Choose from the most used tags', 'ibid' ), 'desc_class' => 'wcfm_full_ele wcfm_side_tag_cloud wcfm_fetch_tag_cloud' )
											), $product_id, $product_type ) );
								} else {
									$product_all_tags = array();
									foreach( $product_defined_tags as $product_defined_tag) {
				...
				<?php if( !apply_filters( 'wcfm_is_category_checklist', true ) ) { ?>
					<div class="wcfm-content">
						<div class="wcfm_product_manager_content_fields">
							...
							$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_product_manage_fields_content', array(
		"excerpt" => array('label' => __('Short Description', 'ibid') , 'type' => $wpeditor, 'class' => 'wcfm-textarea wcfm_ele wcfm_full_ele simple variable external grouped booking ' . $rich_editor , 'label_class' => 'wcfm_title wcfm_full_ele', 'rows' => 5, 'value' => $excerpt, 'teeny' => true),
		"description" => array('label' => __('Description', 'ibid') , 'type' => $wpeditor, 'class' => 'wcfm-textarea wcfm_ele wcfm_full_ele simple variable external grouped booking ' . $rich_editor, 'label_class' => 'wcfm_title wcfm_full_ele', 'rows' => 10, 'value' => $description),
		"pro_id" => array('type' => 'hidden', 'value' => $product_id)
										), $product_id, $product_type ) );
							?>
			...
			if( $wcfm_is_translated_product ) {
				...
				<div class="wcfm-container simple variable external grouped booking">
					<p class="description" style="color:#f86c6b;font-size:15px;margin:10px 15px;width:100%;">
						<?php _e( 'It is a translated product, so product meta fields are blocked for editing because WPML will copy its value from the original language.', 'ibid' ); ?>
					</p>
				</div>
				<div class="wcfm_clearfix"></div><br />
				<?php
            ...
            <?php if (!class_exists('WCFMu')){ ?>
            <?php } ?>


            <?php #Begin: Blue Box iBid auctions hook ?>
            <?php do_action('ibid_wcfm_edit_product_before_tabs', $product_id); ?>
            <?php #End: Blue Box iBid auctions hook ?>


			<div class="wcfm-tabWrap">
			  <div class="wcfm-tabWrap-content <?php echo esc_attr($wcfm_tab_class); ?>">
			...
			<div id="wcfm_products_simple_submit" class="wcfm_form_simple_submit_wrapper">
			  <div class="wcfm-message" tabindex="-1"></div>
				...
				<?php if( $product_id && ( $wcfm_products_single->post_status == 'publish' ) ) { ?>
					<input type="submit" name="submit-data" value="<?php if( apply_filters( 'wcfm_is_allow_publish_live_products', true ) ) { _e( 'Submit', 'ibid' ); } else { _e( 'Submit for Review', 'ibid' ); } ?>" id="wcfm_products_simple_submit_button" class="wcfm_submit_button" />
				<?php } else { ?>
					<?php if( apply_filters( 'wcfm_is_allow_product_limit', true ) && apply_filters( 'wcfm_is_allow_space_limit', true ) ) { ?>
					  <input type="submit" name="submit-data" value="<?php if( apply_filters( 'wcfm_is_allow_publish_products', true ) ) { _e( 'Submit', 'ibid' ); } else { _e( 'Submit for Review', 'ibid' ); } ?>" id="wcfm_products_simple_submit_button" class="wcfm_submit_button" />
					<?php } ?>
				<?php } ?>
				<?php if( apply_filters( 'wcfm_is_allow_draft_published_products', true ) && apply_filters( 'wcfm_is_allow_add_products', true ) ) { ?>
					<input type="submit" name="draft-data" value="<?php _e( 'Draft', 'ibid' ); ?>" id="wcfm_products_simple_draft_button" class="wcfm_submit_button" />
				<?php } ?>
				...
				if( apply_filters( 'wcfm_is_allow_product_preview', true ) ) {
					if( $product_id && ( $wcfm_products_single->post_status != 'publish' ) ) {
						echo '<a target="_blank" href="' . apply_filters( 'wcfm_product_preview_url', get_permalink( $wcfm_products_single->ID ) ) . '">';
						?>
						<input type="button" class="wcfm_submit_button" value="<?php _e( 'Preview', 'ibid' ); ?>" />
						<?php
						echo '</a>';
					} elseif( $product_id && ( $wcfm_products_single->post_status == 'publish' ) ) {
						echo '<a target="_blank" href="' . apply_filters( 'wcfm_product_preview_url', get_permalink( $wcfm_products_single->ID ) ) . '">';
						?>
						<input type="button" class="wcfm_submit_button" value="<?php _e( 'View', 'ibid' ); ?>" />
						<?php
						echo '</a>';
					}
				}
